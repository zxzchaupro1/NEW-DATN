...
const productDec = async (req, res) => {
  try {
    ...
    productPriceAndQuantity = parseInt(
...
const applyCoupon = async (req, res) => {
  try {
    ...
    if (!couponName) {
      ...
      return res
        .status(400)
        .send({ message: "Add Coupon Name", totalAmount, shipping });
    }
    ...
    if (!couponInfo) {
      ...
      return res
        .status(400)
        .send({ message: "Coupon name not valid", totalAmount, shipping });
    }
    ...
    if (couponInfo.ExpiredDate < currentDate) {
      ...
      return res
        .status(400)
        .send({ message: "Coupan has expired", totalAmount, shipping });
    }
